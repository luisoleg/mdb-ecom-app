...
async def get_top_selling_products(
    limit: int = Query(
        10, ge=1, le=50, description="Number of top products to return"
    ),
...
@router.get(
    "/customer-lifetime-value", response_model=List[CustomerLifetimeValue]
)
...
async def get_revenue_trends(
    months: int = Query(
        12, ge=1, le=24, description="Number of months to analyze"
    ),
    current_user_id: str = Depends(get_current_user_id),
) -> Any:
...
async def get_inventory_alerts(
    ...
    pipeline = [
        ...
        {"$unwind": "$variants"},
        {"$match": {"variants.inventory.quantity": {"$lt": threshold}}},
        {
            "$project": {
                "product_name": "$name",
...
async def get_dashboard_summary(
    ...
    recent_stats = (
        recent_revenue[0]
        if recent_revenue
        else {"total_revenue": 0, "total_orders": 0}
    )
    ...
    low_stock = (
        low_stock_count[0]["low_stock_products"] if low_stock_count else 0
    )
