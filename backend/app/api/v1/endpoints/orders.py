    ...
    for item_data in order_data.items:
        ...
        if variant.inventory.quantity < item_data.quantity:
            raise HTTPException(
                ...
                detail=f"Insufficient stock for {product.name}. Available: {variant.inventory.quantity}"
            )
...
async def get_user_orders(
    ...
    skip = (page - 1) * limit
    orders = await Order.find(query).sort(-Order.created_at).skip(skip).limit(limit).to_list()
