...
async def populate_review_response(review: Review) -> ReviewResponse:
    ...
    if user:
        reviewer_name = (
            f"{user.profile.first_name} {user.profile.last_name[0]}."
        )
...
async def get_reviews(
    ...
    if verified_only:
        filters.append(Review.verified_purchase is True)
    if status:
...
async def create_review(
    ...
    if review_data.order_id:
        order = await Order.find_one(
            Order.id == review_data.order_id, Order.user_id == current_user_id
        )
        if order:
