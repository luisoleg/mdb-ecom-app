...
async def merge_carts(
    merge_data: CartMerge, current_user_id: str = Depends(get_current_user_id)
) -> Any:
    """
    ...
    anonymous_cart = await Cart.find_one(
        Cart.session_id == merge_data.session_id
    )

    if not anonymous_cart or anonymous_cart.is_empty():
        # No anonymous cart to merge, return user cart or create new one
        if not user_cart:
            user_cart = await create_cart_for_user_or_session(
