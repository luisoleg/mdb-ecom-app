...
def populate_store_response(store: Store, distance: Optional[float] = None) -> StoreResponse:
    ...
    return StoreResponse(
        ...
        distance=distance,
        available_products=[item.product_id for item in store.inventory if item.quantity > 0],
...
async def search_nearby_stores(
    ...
    radius: int = Query(10000, ge=1, le=100000, description="Search radius in meters"),
    product_id: Optional[str] = Query(None, description="Filter stores with specific product"),
    variant_id: Optional[str] = Query(None, description="Filter stores with specific variant"),
    services: Optional[List[str]] = Query(None, description="Filter by available services"),
    open_now: Optional[bool] = Query(None, description="Filter by stores open now"),
    limit: int = Query(10, ge=1, le=50, description="Maximum number of results")
) -> Any:
    """
    Search for nearby stores using geospatial queries
    
    This endpoint demonstrates MongoDB's geospatial capabilities with $geoNear aggregation
    """
        ...
        "$addFields": {
            "is_open_now": {
                "$let": {
                    ...
                    "in": True  # Simplified for demo - would need complex logic for actual hours
                }
            }
        }
    })
    ...
    for store_data in stores_data:
        ...
        store = Store(**{k: v for k, v in store_data.items() if k != "distance"})
...
async def find_stores_with_product(
    ...
    radius: int = Query(50000, ge=1, le=100000, description="Search radius in meters"),
    ...
    if variant_id:
        pipeline[0]["$geoNear"]["query"]["inventory"]["$elemMatch"]["variant_id"] = variant_id
                ...
                "$filter": {
                    ...
                    "cond": {
                        "$and": [
                            ...
                            {"$gt": ["$$this.quantity", 0]}
                        ] + ([{"$eq": ["$$this.variant_id", variant_id]}] if variant_id else [])
                    }
                }
            }
        }
    ...
    for store_data in stores_data:
        ...
        clean_data = {k: v for k, v in store_data.items() 
...
async def create_store(
    ...
    existing_store = await Store.find_one(Store.store_id == store_data.store_id)
    if existing_store:
        raise HTTPException(
