...
class Review(Document):
    ...
    variant_id: Optional[str] = Field(
        None, description="Specific variant ID (optional)"
    )
    user_id: str = Field(..., description="Reviewer user ID")
    order_id: Optional[str] = Field(
        None, description="Order ID if this is a verified purchase"
    )
    rating: int = Field(
        ..., ge=1, le=5, description="Rating from 1 to 5 stars"
    )
    title: str = Field(
        ..., min_length=1, max_length=100, description="Review title"
    )
    content: str = Field(
        ..., min_length=10, max_length=2000, description="Review content"
    )
    ...
    verified_purchase: bool = Field(
        default=False, description="Whether this is a verified purchase"
    )
    helpful_votes: int = Field(
        default=0, ge=0, description="Number of helpful votes"
    )
    total_votes: int = Field(
        default=0, ge=0, description="Total number of votes"
    )
    images: List[ProductImage] = Field(
        default_factory=list, description="Review images"
    )
    status: str = Field(
        default="pending",
...
class ReviewCreate(BaseModel):
    ...
    product_id: str = Field(..., description="Product ID")
    variant_id: Optional[str] = Field(
        None, description="Variant ID (optional)"
    )
    order_id: Optional[str] = Field(
        None, description="Order ID for verified purchases"
    )
...
class ReviewUpdate(BaseModel):
    ...
    title: Optional[str] = Field(
        None, min_length=1, max_length=100, description="Review title"
    )
    content: Optional[str] = Field(
        None, min_length=10, max_length=2000, description="Review content"
    )
...
class ReviewResponse(BaseModel):
    ...
    verified_purchase: bool = Field(
        ..., description="Verified purchase status"
    )
    ...
    reviewer_avatar: Optional[str] = Field(
        None, description="Reviewer avatar URL"
    )
...
class ReviewStatsResponse(BaseModel):
    ...
    verified_purchases_percentage: float = Field(
...
class ReviewQuery(BaseModel):
    ...
    verified_only: Optional[bool] = Field(
        None, description="Show only verified purchases"
    )
