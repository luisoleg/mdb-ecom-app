...
class CartItem(BaseModel):
    ...
    price: float = Field(..., gt=0, description="Price at time of adding to cart")
    added_at: datetime = Field(default_factory=datetime.utcnow, description="When item was added")


class CartTotals(BaseModel):
    """Cart totals calculation"""
    items_count: int = Field(default=0, ge=0, description="Total number of items")
    subtotal: float = Field(default=0.0, ge=0, description="Subtotal of all items")
    estimated_tax: float = Field(default=0.0, ge=0, description="Estimated tax")
    estimated_shipping: float = Field(default=0.0, ge=0, description="Estimated shipping")
    estimated_total: float = Field(default=0.0, ge=0, description="Estimated total")


class Cart(Document):
    """Shopping cart document model"""
    user_id: Optional[str] = Field(None, description="User ID (null for anonymous carts)")
    ...
    totals: CartTotals = Field(default_factory=CartTotals, description="Cart totals")
    ...
    def set_expiry(cls, v):
        if not v:
            return datetime.utcnow() + timedelta(days=7)  # Cart expires in 7 days
        return v

    def add_item(self, product_id: str, variant_id: str, quantity: int, price: float):
    ...
    def update_item_quantity(self, product_id: str, variant_id: str, quantity: int):
    ...
    def calculate_totals(self):
        ...
        self.totals.subtotal = sum(item.price * item.quantity for item in self.items)
