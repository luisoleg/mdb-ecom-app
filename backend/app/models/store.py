...
class StoreHours(BaseModel):
    ...
    is_closed: bool = Field(
        default=False, description="Whether store is closed this day"
    )
...
class StoreAddress(BaseModel):
    ...
    state: str = Field(
        ..., min_length=1, max_length=100, description="State/Province"
    )
    postal_code: str = Field(
        ..., min_length=1, max_length=20, description="Postal code"
    )
    country: str = Field(
        ..., min_length=2, max_length=2, description="Country code"
    )
...
class Store(Document):
    ...
    store_id: Indexed(str, unique=True) = Field(
        ..., description="Unique store identifier"
    )
    name: str = Field(
        ..., min_length=1, max_length=100, description="Store name"
    )
    address: StoreAddress = Field(..., description="Store address")
    location: Location = Field(..., description="Store geographic coordinates")
    contact: StoreContact = Field(
        default_factory=StoreContact, description="Contact information"
    )
    hours: Dict[str, StoreHours] = Field(
        ..., description="Operating hours by day of week"
    )
    services: List[str] = Field(
        default_factory=list, description="Available services"
    )
    inventory: List[StoreInventoryItem] = Field(
        default_factory=list, description="Store inventory"
    )
    manager_id: Optional[str] = Field(
        None, description="Store manager user ID"
    )
    capacity: Optional[int] = Field(None, description="Store capacity/size")
    is_active: bool = Field(
        default=True, description="Whether store is active"
    )
    ...
    def get_distance_from(self, longitude: float, latitude: float) -> float:
        ...
        lon2, lat2 = radians(self.location.coordinates[0]), radians(
...
class StoreCreate(BaseModel):
    ...
    store_id: str = Field(
        ..., min_length=1, description="Unique store identifier"
    )
    name: str = Field(
        ..., min_length=1, max_length=100, description="Store name"
    )
    ...
    contact: Optional[StoreContact] = Field(
...
class StoreSearchQuery(BaseModel):
    ...
    radius: int = Field(
        default=10000, ge=1, le=100000, description="Search radius in meters"
    )
...
class StoreListResponse(BaseModel):
    ...
    search_center: Optional[Dict[str, float]] = Field(
        None, description="Search center coordinates"
    )
    search_radius: Optional[int] = Field(
        None, description="Search radius in meters"
    )
...
class NearbyStoreResponse(BaseModel):
    ...
    stores: List[StoreResponse] = Field(
        ..., description="Nearby stores with the product"
    )
