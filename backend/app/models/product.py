...
class InventoryInfo(BaseModel):
    ...
    warehouse_location: Optional[str] = Field(None, description="Warehouse location code")
...
class ProductVariant(BaseModel):
    ...
    attributes: Dict[str, str] = Field(default_factory=dict, description="Variant attributes (color, size, etc.)")
    inventory: InventoryInfo = Field(..., description="Inventory information")
    images: List[ProductImage] = Field(default_factory=list, description="Variant-specific images")
...
class Product(Document):
    ...
    base_price: float = Field(..., gt=0, description="Base price for the product")
    variants: List[ProductVariant] = Field(..., min_items=1, description="Product variants")
    specifications: Dict[str, Any] = Field(default_factory=dict, description="Product specifications")
    seo: SEOInfo = Field(default_factory=SEOInfo, description="SEO information")
    ...
    def get_price_range(self) -> tuple[float, float]:
        ...
        prices = [variant.price for variant in self.variants if variant.is_active]
        if not prices:
            return (0.0, 0.0)
    ...
    def is_in_stock(self) -> bool:
        ...
        return any(variant.inventory.quantity > 0 for variant in self.variants if variant.is_active)
...
class Category(Document):
    ...
    slug: Indexed(str, unique=True) = Field(..., description="URL-friendly category slug")
    ...
    path: str = Field(..., description="Category path (e.g., /electronics/audio)")
    children: List[str] = Field(default_factory=list, description="Child category IDs")
...
class ProductSearchQuery(BaseModel):
    ...
    sort_by: Optional[str] = Field("relevance", description="Sort by: relevance, price_asc, price_desc, rating, newest")
